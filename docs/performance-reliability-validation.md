...
- When CDP is unavailable, use `/api/tv-diagnostics` `ui.focus`, `ui.selectedTitle`, `ui.selectedItem`, and `ui.selectedDetail` to confirm remote-navigation state before pressing Enter.
- On Windows, posting `WM_KEYDOWN`/`WM_KEYUP` to the `emulator-x86_64` `Tizen Emulator` window has been a reliable fallback for remote keys after `sdb shell input keyevent` is ignored.
- Use `scripts/send-tizen-emulator-key.ps1` for repeatable Windows key-message input during emulator validation, for example `.\scripts\send-tizen-emulator-key.ps1 Right Enter Back`.
- Use `scripts/measure-tv-idle.ps1` for idle soak evidence. The one-hour pass should record inactive playback activity, continuing diagnostics heartbeat, browser memory from `/api/tv-diagnostics`, and host emulator memory before and after the run.
